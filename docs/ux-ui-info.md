...
- **Sombras (Elevation):** Muy contenidas. Solo se usa `shadow-sm` en las tarjetas blancas sobre el fondo `zinc-50` para separarlas sutilmente (excluyendo las Data Tables que, por estándar visual, carecen de sombra). Modales y Drawers usan `shadow-xl`.

## 4. ESTÁNDARES DE COMPONENTES Y LAYOUT

La plataforma utiliza patrones de UI estandarizados para mantener consistencia 100% entre todas las vistas de gestión.

- **Page Layout Wrapper**: Todas las vistas de escritorio principales deben utilizar un wrapper que permita la mayor expansión y asigne el padding en base 6.
  Ejemplo: `<div className="h-full w-full overflow-y-auto bg-zinc-50 p-6"><div className="w-full mx-auto pb-20">`
- **Data Tables (Estandarización)**: El componente nativo `table.tsx` inyecta automáticamente la estética de tablas (background blanco, border rounded-xl sin sombra, padding px-6 py-4 y font-size uniformes). Prohibido envolver la `<Table>` en tarjetas adicionales que inyecten `shadow-sm`, y prohibido sobreescribir estilos y tamaños de letra inline dentro de `<TableCell>` en cada página.
- **Toolbars (Filtros y Búsqueda)**: Controles "sin contenedor padre". Flotan directo sobre el `bg-zinc-50`. Input de búsqueda con logo dentro (`pl-10`), botones de filtro en `variant="outline"` a la derecha (Categoría, Calidad, Ordenar).

## 5. ESTADOS DE INTERACCIÓN Y CARGA (UX)

### 5.1. Skeletons (Pantallas de Carga)
...
### 5.2. Estados Vacíos (Empty States)
...
### 5.3. Feedback Inmediato (Toasts y Botones)
...
## 6. INVENTARIO DE PANTALLAS Y COMPONENTES (Screens)
...
### 6.1. Matriz de Pedidos (Checkout B2B)
...
### 6.2. Reporte de Pago Manual (Drawer Lateral)
...
### 6.4. Tablero de Preparación (Kanban Logístico)
...
### 6.5. Verificación de Comprobante (Tesorería)
